...
def run_training(tol, epsilon):
    ...
    while True:
        mse = 0
        num_batches = int(np.ceil(X_train.shape[0] / config.MINI_BATCH_SIZE))

        for batch in range(num_batches):
            start_idx = batch * config.MINI_BATCH_SIZE
            end_idx = min((batch + 1) * config.MINI_BATCH_SIZE, X_train.shape[0])
            ...
            L = (1 / 2) * np.mean((Y_batch - h[config.NUM_LAYERS - 1]) ** 2)
            mse += L * X_batch.shape[0]

            del_L_by_del_h[config.NUM_LAYERS - 1] = (h[config.NUM_LAYERS - 1] - Y_batch)

            for l in range(config.NUM_LAYERS - 2, 0, -1):
                del_L_by_del_h[l] = np.matmul(del_L_by_del_h[l + 1], (del_fl_by_del_z[l + 1] * pl.theta[l + 1]).T)

            for l in range(1, config.NUM_LAYERS):
                del_L_by_del_theta0[l] = np.mean(del_L_by_del_h[l] * del_hl_by_del_theta0[l], axis=0, keepdims=True)
                del_L_by_del_theta[l] = np.matmul(h[l - 1].T, del_L_by_del_h[l] * del_fl_by_del_z[l])
